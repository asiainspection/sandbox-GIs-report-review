...
when: null
check:
  - extract("Quote the sentence listing defects broken down by PO number with total defect percentage, or null")
  - matches("PO\\s*\\S+.*\\d+(\\.\\d+)?%")
```
...
when: report.inspection_type equals "Pre-Shipment Inspection"
...
where: [report.global_remark]
when: null
check: extract_bool("Does this remark mention a quantity shortage or overage compared to the ordered quantity?")
```
...
where:
  - kind: checklist
    match: [outer, packing, shipping, marks]
    field: comment
when: null
check: null
```
...
where:
  - kind: checklist
    ...
    field: comment
when: null
check: extract_bool("Does this comment confirm an anti-mold sheet, desiccant pack, or vent hole is present?")
```
...
where:
  - kind: checklist
    match: [outer, packing, assortment, dimensions, weight]
    field: photo_count
when: null
check: count_at_most(0)
```
...
when: null
check: extract_bool("Does this text mention swift attaches on serial tags being clear or transparent instead of black?")
```
...
where: [report.inspector_text]
when: null
check: extract_bool("Does this text mention joker tags being attached with plastic swift attaches instead of sewn with thread?")
```
...
```check
where: [workmanship.measurement_aql]
when: null
check: equals(4.0)
```
